  ...
  document.querySelectorAll('input[type="date"]').forEach(input => {
    ...
    btn.addEventListener('click', function () {
      /* showPicker() requires transient user activation; a dwell
         ...
         and say why, rather than leaving an unexplained focus jump
         after activating a control named "Show date picker for …". */
      try {
        input.showPicker();
      } catch (e) {
        input.focus();
        liveSay('The calendar cannot be opened this way. Type the date, '
          + 'or use the step buttons below the field.');
      }
    });
    row.appendChild(btn);
  });
...
/* Grace added on top of the repeat interval before an auto-repeating
   stepper announces, so a burst of steps speaks once at its settled
   value instead of flooding the live region. */
const DWELL_ANNOUNCE_DELAY_MS = 400;
/* Trailing delay for a discrete step (click or key press): long enough
   to coalesce a quick double-press, short enough to feel immediate. */
const DWELL_ANNOUNCE_SETTLE_MS = 150;
...
function wireDwell() {
  ...
  let repeating = false; /* true only while an auto-repeat step is firing */
  ...
  const fireDwell = () => {
    ...
    if (el.classList.contains('dwell-stepper')) {
      /* Stepper buttons auto-repeat: parking the pointer steps the value
         ...
         a date can be walked forward week by week hands-free. The flag
         tells announceValue to hold its announcement until the stepping
         stops rather than speaking every intermediate value. */
      repeating = true;
      activate(el);
      repeating = false;
      armTarget(el);
  ...
  const buildSteppers = () => {
    /* Steppers auto-repeat while the pointer rests on them, and a
       screen reader or braille display can take longer to speak a date
       than the repeat interval — leaving the user trailing seconds
       behind the real value. During auto-repeat the announcement waits
       out the repeat interval, so a burst of steps speaks once, at the
       settled value; a discrete click or key press still announces
       promptly. The visible value updates on every step either way. */
    ...
    const announceValue = (input) => {
      ...
      const delay = repeating
        ? seconds * 1000 + DWELL_ANNOUNCE_DELAY_MS
        : DWELL_ANNOUNCE_SETTLE_MS;
    ...
    /* labelTemplate carries %s where the field name goes, so the
       button can be re-labelled if that name changes with mode. The
       visible glyph is prepended to the accessible name (SC 2.5.3
       Label in Name): a voice-control user says "click minus 10", so
       the name must literally contain the "−10" shown on the button. */
    const makeBtn = (group, input, text, labelTemplate, handler) => {
      const b = createEl('button', 'dwell-stepper', text);
      b.type = 'button';
      setGeneratedLabel(b, input, text + ': ' + labelTemplate);
    ...
    document.querySelectorAll('input[type="number"], input[type="date"]')
      .forEach(input => {
        ...
        if (input.type === 'number') {
          ...
          /* Date templates read "−1w: Leave start date 1 week earlier". */
        }
